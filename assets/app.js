...
/* ── Deferred-feature entry points ──────────────────────────────────────────
   What-if, comparison and the CSV export live in app-extra.js. None is
   reachable on first paint — two are dialogs, one is a download behind a
   button — but they had drifted into the core bundle, which the README already
   described as not carrying them. Each entry point is a thin shim so callers
   do not each have to know about the split.
   ─────────────────────────────────────────────────────────────────────────── */
/** Draw the what-if grid. @param {Object} cp */
function renderWhatIf(cp){
  if(typeof _renderWhatIfImpl==='function')return _renderWhatIfImpl(cp);
  withExtras(function(){ _renderWhatIfImpl(cp) });
}
/** Refresh the what-if figures without rebuilding the grid. */
function updateWiResults(){
  if(typeof _updateWiResultsImpl==='function')return _updateWiResultsImpl();
}
/** Open the comparison dialog against a history entry. @param {number} i */
function openCompare(i){
  if(typeof _openCompareImpl==='function')return _openCompareImpl(i);
  withExtras(function(){ _openCompareImpl(i) });
}
/** Redraw the comparison, if it happens to be open. */
function renderCompare(){
  if(typeof _renderCompareImpl==='function')return _renderCompareImpl();
}
/** Download the history as CSV. */
function exportHistoryCSV(){
  if(typeof _exportHistoryCSVImpl==='function')return _exportHistoryCSVImpl();
  withExtras(function(){ _exportHistoryCSVImpl() });
}
